...
__global__ void linearCombinationKernel(int nv, T* c, T** xd, T* zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    zd[i] = c[0] * xd[0][i];
    for (int j = 1; j < nv; j++) zd[i] += c[j] * xd[j][i];
  }
}
 ...
 */
template<typename T, typename I>
__global__ void scaleAddMultiKernel(int nv, T* c, T* xd, T** yd, T** zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++) zd[j][i] = c[j] * xd[i] + yd[j][i];
  }
}
 ...
 */
template<typename T, typename I>
__global__ void linearSumVectorArrayKernel(int nv, T a, T** xd, T b, T** yd,
                                           T** zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++) zd[j][i] = a * xd[j][i] + b * yd[j][i];
  }
}
 ...
 */
template<typename T, typename I>
__global__ void scaleVectorArrayKernel(int nv, T* c, T** xd, T** zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++) zd[j][i] = c[j] * xd[j][i];
  }
}
 ...
 */
template<typename T, typename I>
__global__ void constVectorArrayKernel(int nv, T c, T** zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++) zd[j][i] = c;
  }
}
 ...
 */
template<typename T, typename I>
__global__ void scaleAddMultiVectorArrayKernel(int nv, int ns, T* c, T** xd,
                                               T** yd, T** zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
 ...
 */
template<typename T, typename I>
__global__ void linearCombinationVectorArrayKernel(int nv, int ns, T* c, T** xd,
                                                   T** zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int k = 0; k < nv; k++)
    {
      ...
      for (int j = 1; j < ns; j++) zd[k][i] += c[j] * xd[k * ns + j][i];
    }
  }
}
